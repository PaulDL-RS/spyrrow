...
impl ProgressQueuePy {
    ...
    ///
    /// Returns:
    ///     list[tuple[str, float, float]]: A list of (phase, strip_width, density) tuples.
    ///       Phase is one of "exploring", "compressing", or "final".
    ///
    fn drain(&self) -> Vec<(String, f32, f32)> {
        let mut queue = self.inner.lock().unwrap();
        queue.drain(..).map(|r| (r.phase, r.strip_width, r.density)).collect()
    }
}
...
impl SolutionListener for ProgressListener {
    fn report(&mut self, report: ReportType, solution: &SPSolution, instance: &SPInstance) {
        let phase = match report {
            ReportType::ExplFeas | ReportType::ExplInfeas | ReportType::ExplImproving => "exploring",
            ReportType::CmprFeas => "compressing",
